...
### 핵심 주장
1. TDA는 유클리드 분석과 **완전히 직교하는** 유전자 정보를 발견한다 (Top 200 겹침 = 0)
...
- **BRCA 서브셋**: 1,218 샘플 (종양 1,105 + 정상 113)
...
> **Figure: fig2_persistence_diagrams.pdf**
...
> **Figure: fig3_statistical_validation.pdf**
...
> **Figure: fig4_gene_discovery.pdf**
...
> **Figure: fig6_classification.pdf**
...
> **Figure: fig5_pathway_comparison.pdf**
...
TCGA RNA-seq (TPM)
  ...
  → H2C gene panel selection (TDA Top 200 ∩ Euclidean p > 0.05)
  → GO/KEGG pathway enrichment (Enrichr)
  → 5-fold CV classification validation
```
...
모든 Figure는 `phase5_visualization_paper/figures/`에 PNG + PDF(벡터) 양쪽으로 저장.
...
| 한계 | 설명 | 해결 방안 |
|------|------|----------|
| 단일 데이터셋 | TCGA-BRCA만 사용 | 독립 BRCA 코호트 외부 검증 |
| 단일 암종 | BRCA만 분석 | TCGA 내 다른 암종(LUAD, COAD 등)으로 확장 |
| 정상 샘플 부족 | 113개 (전체의 9.3%) | Bootstrap으로 보완했으나 근본적 한계 |
| 실험적 검증 없음 | 컴퓨터 분석만 수행 | Wet-lab functional assay 필요 |
