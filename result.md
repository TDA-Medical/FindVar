...
- float32 정밀도, in-place 연산 (`.sub_()`, `.div_()`)으로 메모리 절감
- 유전자 청킹 불필요 (전체 ~21,000 유전자를 한 번에 처리 → 정확한 EB 추정)
- CPU 대비 ~300배 속도 향상 (**15분 → 2.8초**)

### 최종 입력 데이터
- `cleaned_tcga_tpm_for_TAE.csv`: **1,215 샘플 × 20,862 유전자**

### 탐색적 데이터 분석 (EDA, 전처리 단계에서 수행)
...
→ 이 결과가 Phase 3에서 TDA 발견과의 **교차 검증 기준선**이 됨

### 엔지니어링 문제 해결 (Data-preprocessing 과정에서)
...
1. 미니배치 내 pairwise distance matrix 계산:
   - Euclidean: `D_ij = sqrt(Σ_k (x_ik - x_jk)² + eps)`
   - Cosine: `D_ij = 1 - cos(x_i, x_j)` (각도 기반, 크기 불변)
   - Pearson: `D_ij = 1 - r(x_i, x_j)` (평균중심화 후 cosine)
2. 양쪽 distance matrix를 [0, 1]로 정규화
3. `L_topo = MSE(D_norm_latent, D_norm_original)`
...
- **Debiased divergence**: `OT(orig, lat) - 0.5×OT(lat, lat) - 0.5×OT(orig, orig) ≥ 0`
- **Envelope Theorem**: 수렴 후 dual variable을 detach → VRAM ~50배 절감
...
#### 탐색 결과: H1 고리(루프) 수 비교 (원본 전체, 크기 매칭 전)
...
세 가지 검증을 수행했다:
1. **Permutation Test**: 종양+정상을 합친 후 113 vs 113으로 랜덤 분할, 500회
2. **H1 Count Test**: 종양에서 113개 서브샘플 추출 200회, 정상과 H1 수 비교
3. **Bootstrap Stability**: 정상 113개를 복원추출 200회, PH 안정성 확인

#### 핵심 결과: H1 고리 수 (크기 매칭, 113 vs 113)
...
#### Permutation Test 결과 (Wasserstein/Bottleneck)
...
#### Bootstrap Stability (정상 113개 리샘플링)
...
→ H0는 안정적(CV ~5%). H1은 변동 크지만(CV ~44%), 종양(10~30)과 정상(0~7) 분포가 완전 분리.
...
#### H2C Gene Panel: TDA Top 200 중 유클리드 비유의미(p>0.05) 37개
...
#### 4-3. Cross-Cancer Validation (Pan-Cancer)
...
| Figure | 내용 | 파일 | Phase |
|--------|------|------|-------|
| Fig 2 | Persistence Diagram: 전체/종양/정상 비교 | `fig2_persistence_diagrams.pdf` | 1 |
| Fig 3 | 통계 검증: H1 count test + H0 bootstrap | `fig3_statistical_validation.pdf` | 2 |
| Fig 4 | 유전자 발견: Top 20 + TDA vs 유클리드 산점도 | `fig4_gene_discovery.pdf` | 3 |
| Fig 5 | Pathway 비교: TDA vs 유클리드 | `fig5_pathway_comparison.pdf` | 4 |
| Fig 6 | 분류 성능: AUC 비교 + Feature vs 성능 | `fig6_classification.pdf` | 4 |
| Fig 7 | Latent Space: PCA + Top 차원 boxplot | `fig7_latent_space.pdf` | 3 |
| Fig 8 | Cross-cancer: H2C pan-cancer AUC | `cross_cancer_validation.pdf` | 4 |
| Summary | 전체 결과 요약 (6패널) | `summary_figure.pdf` | 5 |
